...
class VisitCountries extends Component {
  ...
  OnChangeVisitStatus = id => {
    ...
    const modifiedCountriesList = countriesList.map(eachCountry => {
      if (eachCountry.id === id) {
        return {...eachCountry, isVisited: !eachCountry.isVisited}
      }
      return eachCountry
    })
    this.setState({countriesList: modifiedCountriesList})
  }
          ...
          <CountriesItemList>
            {countriesList.map(eachCountry => (
              <CountriesItem
                ...
                onVisitCountry={this.OnChangeVisitStatus}
              />
            ))}
          </CountriesItemList>
        </CountriesListContainer>
        <VisitedCountryContainer>
          <SectionHeading>Visited Countries</SectionHeading>
            ...
            <VisitedCountryItemList>
              {visitedCountries.map(eachCountry => (
                <Countries
                  key={eachCountry.id}
                  countryDetails={eachCountry}
                  onRemoveCountry={this.OnChangeVisitStatus}
                />
              ))}
            </VisitedCountryItemList>
          ) : (
